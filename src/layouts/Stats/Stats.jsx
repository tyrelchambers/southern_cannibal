...
const getStats = async (setStats) => {
  const link = `https://www.googleapis.com/youtube/v3/channels?part=snippet,contentDetails,statistics&id=${
    import.meta.env.VITE_CHANNEL_ID
  }&key=${import.meta.env.VITE_YOUTUBE_API}`;
  const stats = await Axios.get(link).then(
    (res) => res.data.items[0].statistics
  );
  setStats({
